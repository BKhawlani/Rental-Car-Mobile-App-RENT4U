...
import 'package:rental_car_project/screen/CarDetails.dart' show Cardetails;
import 'package:rental_car_project/utilities/car.dart';
import 'package:http/http.dart' as http;
import 'dart:convert';

class Sportcar extends StatefulWidget {
  const Sportcar({super.key});

  @override
  State<Sportcar> createState() => _SedancarState();
}

class _SedancarState extends State<Sportcar> {
  Future<List<Car>> fetchCars() async {
    try {
      final resportcarnse = await http.get(
        Uri.parse(
          'https://680c930b2ea307e081d45573.mockapi.io/rent4u/api/cars',
        ),
      );
      if (resportcarnse.statusCode == 200) {
        List<dynamic> data = json.decode(resportcarnse.body);
        return data.map((car) => Car.fromJson(car)).toList();
      } else {
        // إذا السيرفر رد بكود خطأ (مثلا 404، 500...)
        if (kDebugMode) {
          print('Server error: ${resportcarnse.statusCode}');
        }
        return []; // مصفوفة فاضية
      }
    } catch (e) {
  ...
  void initState() {
    ...
    fetchCars().then((carList) {
      setState(() {
        cars = carList;
        sportcar = cars
            .where((car) => car.category == "Sports Car" && car.isAvailable)
            .toList();
        isloading = false;
        isfetched = true;
      });
                          ...
                          MaterialButton(
                            onPressed: () {
                              Navigator.of(context).push(MaterialPageRoute(
                                  builder: (context) =>
                                      Cardetails(car: sportcar, index: i)));
                            },
                            child: Container(
                              ...
                              child: Column(
                                children: [
                                  ...
                                  Text(
                                    sportcar[i].brand + " " + sportcar[i].model,
                                    style: GoogleFonts.poppins(
                                      fontSize: fontSizeSubtitle,
